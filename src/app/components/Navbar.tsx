...
//         {/* Mobile Dropdown */}
//         {open && (
//           <div className="md:hidden bg-[#7AC043] relative z-[99999] px-4 py-3 space-y-2">
...
export default function Navbar() {
  ...
  const toggleMobileMenu = (key: string) => {
    setMobileOpenMenu(mobileOpenMenu === key ? null : key);
  };
  ...
  return (
    <header>
      ...
      <nav className="bg-[white] shadow-lg h-[130px]">
        ...
        {/* Mobile Dropdown */}
        {open && (
          <div className="md:hidden bg-[#7AC043] relative z-[99999] px-4 py-3 space-y-1 max-h-[70vh] overflow-y-auto">
            <Link href="/" className="block text-white hover:text-[#FFB706] py-2 font-poppins">Home</Link>

            {/* Courses mobile mega menu */}
            <div className="border-b border-white/20">
              <div className="flex items-center justify-between">
                <Link href="/all-courses" className="block text-white hover:text-[#FFB706] py-2 font-poppins">Courses</Link>
                <button
                ...
                >
                  <ChevronDownIcon
                    className={`h-4 w-4 transition-transform ${mobileOpenMenu === "courses" ? "rotate-180" : ""}`}
                  />
                </button>
              </div>
              {mobileOpenMenu === "courses" && (
                <div className="pl-4 pb-2 space-y-1">
                  {courses.map((option, idx) => (
                    <Link
                      key={idx}
                      href={option.href}
                      className="block text-white/90 hover:text-[#FFB706] py-1 font-jakarta text-sm"
                    >
                      {option.label}
                    </Link>
                  ))}
            ...
            <Link href="/about-us" className="block text-white hover:text-[#FFB706] py-2 font-poppins">About</Link>

            {/* Online Learning mobile mega menu */}
            <div className="border-b border-white/20">
              <div className="flex items-center justify-between">
                <span className="block text-white py-2 font-poppins">Online Learning</span>
                <button
                  onClick={() => toggleMobileMenu("uk")}
                  className="p-2 text-white"
                  aria-label="Toggle Online Learning submenu"
                >
                  <ChevronDownIcon
                    className={`h-4 w-4 transition-transform ${mobileOpenMenu === "uk" ? "rotate-180" : ""}`}
                  />
                </button>
              </div>
              {mobileOpenMenu === "uk" && (
            ...
            <Link href="/blogs" className="block text-white hover:text-[#FFB706] py-2 font-poppins">Blogs</Link>

            {/* Teachers mobile mega menu */}
            <div className="border-b border-white/20">
              <div className="flex items-center justify-between">
                <Link href="/all-teachers" className="block text-white hover:text-[#FFB706] py-2 font-poppins">Teachers</Link>
                <button
                  onClick={() => toggleMobileMenu("teachers")}
                  className="p-2 text-white"
                  aria-label="Toggle Teachers submenu"
                >
                  <ChevronDownIcon
                    className={`h-4 w-4 transition-transform ${mobileOpenMenu === "teachers" ? "rotate-180" : ""}`}
                  />
                </button>
              </div>
              {mobileOpenMenu === "teachers" && (
                <div className="pl-4 pb-2 space-y-1">
                  {teachers.map((option, idx) => (
                    <Link
                      key={idx}
            ...
            <Link href="/registration" className="block text-white hover:text-[#FFB706] py-2 font-poppins">Registration</Link>
            <Link href="/contact-us" className="block text-white hover:text-[#FFB706] py-2 font-poppins">Contact Us</Link>
          </div>
        )}
      </nav>
    </header>
